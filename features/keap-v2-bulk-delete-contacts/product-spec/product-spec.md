...
| FR-015 | <!-- CR-002/BUG-005 --> The destructive tool is OPT-IN: registered + dispatched only when `KEAP_BULK_DELETE_ENABLED=true` (default OFF in prod), so existing OAuth clients don't silently gain delete power. | Must |
| FR-017 | <!-- CR-003 --> Even when enabled, a real delete runs ONLY if the call's `confirm` arg equals server env `KEAP_BULK_DELETE_CONFIRM` (a value NOT in the model's context). Missing/wrong `confirm` → refused (no deletes). `dry_run` is exempt (previews need no confirm). Protects against an autonomous LLM invoking deletes without explicit human authorization per batch. | Must |
| FR-016 | <!-- CR-002/BUG-004 --> The admin `/admin/mint-client` secret is read from the `x-approval-secret` header, never the query string (avoids leak to logs/history). | Must |
| FR-014 | <!-- CR-001/BUG-001; CR-002/BUG-006: 30s timeout scoped to deleteV2 only, NOT a global requestV2 default --> The rate limiter is PROCESS-GLOBAL (shared across all concurrent invocations in a worker instance). <!-- CR-001 addendum: --> Additionally, the Worker transport injects a CROSS-INSTANCE limiter backed by a Durable Object (`KeapRateLimiter`, single global id) so overlapping calls across worker isolates share one 25 req/s/app budget. stdio falls back to the process-global limiter. | Must |
